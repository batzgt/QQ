...
from iqpilot.selfdrive.ui.lib.local_routes import local_route_audio_paths
...
from iqpilot.selfdrive.ui.lib.local_routes import (
  CAMERA_LABELS,
...
def _find_hwdec() -> str | None:
  ...
  try:
    ...
    path = os.path.join(BASEDIR, "iqpilot", "system", "loggerd", "encoder", "v4l_decode")
...
class VideoPlayerLayout(Widget):
  ...
  def __init__(self):
    super().__init__()
    self._header = self._child(ScreenHeader(lambda: tr("Video Player")))
