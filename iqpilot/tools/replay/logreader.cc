...
bool LogReader::load(const char *data, size_t size, std::atomic<bool> *abort) {
  ...
  try {
    ...
    rWarning("Failed to parse log. Retrieved %zu events from corrupt log", events.size());
  }
  ...
  if (requires_migration) {
    try {
      ...
      rWarning("Failed to migrate corrupt log: %s", e.getDescription().cStr());
    } catch (...) {
      rWarning("Failed to migrate corrupt log");
    }
  }
