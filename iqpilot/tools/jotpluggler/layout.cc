...
DbcEditorSource resolve_dbc_editor_source(const std::string &dbc_name) {
  const fs::path generated_dbc_dir = repo_root() / "iqpilot" / "tools" / "jotpluggler" / "generated_dbcs";
  const std::array<DbcEditorSource, 2> candidates = {{
    {.path = fs::path(IQDBC_DBC_PATH) / (dbc_name + ".dbc"), .kind = DbcEditorState::SourceKind::Opendbc},
