...
std::vector<std::string> available_dbc_names_impl() {
  ...
  for (const fs::path &dbc_dir : {
         fs::path(IQDBC_DBC_PATH),
         repo_root() / "iqpilot" / "tools" / "jotpluggler" / "generated_dbcs",
       }) {
...
fs::path resolve_dbc_path(const std::string &dbc_name) {
  for (const fs::path &candidate : {
         fs::path(IQDBC_DBC_PATH) / (dbc_name + ".dbc"),
         repo_root() / "iqpilot" / "tools" / "jotpluggler" / "generated_dbcs" / (dbc_name + ".dbc"),
       }) {
