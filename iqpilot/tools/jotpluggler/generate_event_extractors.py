...
class Generator:
  ...
  def emit_can_special(self, indent, service_name):
    ...
    # IQ.Pilot patch: iqpilot's cereal CanData keeps the deprecated busTime as a flat
    # `busTimeDEPRECATED` field rather than upstream's `deprecated :group { busTime }`.
    self.emit(indent + 2, "append_can_frame(can_service, static_cast<uint8_t>(msg.getSrc()), msg.getAddress(), msg.getBusTimeDEPRECATED(), msg.getDat(), tm, series);")  # noqa: E501
...
if __name__ == "__main__":
  if len(sys.argv) != 4:
    print(f"usage: {sys.argv[0]} <repo-root> <iqdbc-root> <output>", file=sys.stderr)
    sys.exit(2)

  repo_root = Path(sys.argv[1]).resolve()
  iqdbc_root = Path(sys.argv[2]).resolve()
  output = Path(sys.argv[3])
  capnp.remove_import_hook()
  log = capnp.load(str(repo_root / "iqpilot" / "cereal" / "log.capnp"), imports=[str(iqdbc_root / "car")])
