...
from iqpilot.system.qcomgpsd.modemdiag import ModemDiag, DIAG_LOG_F, setup_logs, send_recv
from iqpilot.system.qcomgpsd.structs import (dict_unpacker, position_report, relist,
...
def teardown_quectel(diag):
  try:
    ...
    cloudlog.exception("failed to disable quectel outport during teardown")
  try:
    if gps_enabled():
      at_cmd("AT+QGPSEND")
  except Exception:
    cloudlog.exception("failed to stop quectel gps during teardown")
  try:
    try_setup_logs(diag, [])
  except Exception:
    cloudlog.exception("failed to clear quectel logs during teardown")


def close_diag(diag: ModemDiag | None) -> None:
  if diag is None:
    return
  try:
    diag.serial.close()
  except Exception:
    cloudlog.exception("failed to close modem diag serial")


def ensure_quectel_setup(diag: ModemDiag | None, reason: str) -> tuple[ModemDiag, bool]:
  while True:
    try:
      close_diag(diag)
      wait_for_modem()
      diag = ModemDiag()
      assisted = setup_quectel(diag)
      cloudlog.warning(f"quectel setup done ({reason})")
      return diag, assisted
    except Exception:
      cloudlog.exception(f"quectel setup failed during {reason}, retrying")
      close_diag(diag)
      time.sleep(1.0)
...
def main() -> NoReturn:
  ...
  # connect to modem
  diag = None
  diag, r = ensure_quectel_setup(diag, "startup")
  ...
  while 1:
    if os.path.exists(ASSIST_DATA_FILE) and want_assistance:
      diag, r = ensure_quectel_setup(diag, "assistance_reload")
      want_assistance = not r

    try:
      opcode, payload = diag.recv()
    except Exception:
      cloudlog.exception("modem diag receive failed, resetting qcomgpsd session")
      diag, r = ensure_quectel_setup(diag, "diag_receive_recovery")
      want_assistance = want_assistance and not r
      continue
    ...
    if log_type == LOG_GNSS_OEMDRE_MEASUREMENT_REPORT:
      ...
      gps.flags = 1 if gps.hasFix else 0
      if gps.hasFix:
