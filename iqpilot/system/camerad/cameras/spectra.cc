...
void SpectraCamera::camera_open(VisionIpcServer *v, cl_device_id device_id, cl_context ctx) {
  ...
  camera_map_bufs();
  clearAndRequeue();
  running_cameras.insert(cc.camera_num);
}
...
// mici (comma 4) driver-cam HDR: read OS04C10 full-res and 2x-downscale in the BPS.
// Verbatim port of upstream commaai/openpilot config_bps (#37876 + gamma/BLC baseline).
// Only ever called for out_scale>1 (OS04C10 index 3); comma 3/3x use the original config_bps.
void SpectraCamera::config_bps_downscale(int idx, int request_id) {
  /*
    Handles per-frame BPS config.
    * BPS = Bayer Processing Segment
  */
...
void SpectraCamera::enqueue_frame() {
  ...
  if (cc.output_type == ISP_BPS_PROCESSED) {
    // mici (comma 4) driver cam reads full-res and downscales in the BPS; everything else
    // (comma 3/3x driver cam) keeps the original binned BPS path untouched.
    if (sensor->out_scale > 1) config_bps_downscale(i, request_id);
    else config_bps(i, request_id);
  }
}
...
void SpectraCamera::configICP() {
  ...
  */
  ...
  // striping lib output. The mici driver-cam downscale program (out_scale>1) is larger (0xc58),
  // and needs a bigger BPSStripingLib scratch buffer (0xcfe0) plus linearization/gamma LUTs. Every
  // other BPS camera (comma 3/3x driver cam) keeps the original 0x9a0/0xa100 sizes and no LUTs.
  bool bps_downscale = sensor->out_scale > 1;
  uint32_t striping_size = bps_downscale ? (sizeof(bps_striping_output[0]) / sizeof(bps_striping_output[0][0])) : 0x9a0;
  ...
  bps_cdm_striping_bl.init(m, bps_downscale ? 0xcfe0 : 0xa100, 0x20, true, m->icp_device_iommu);

  if (bps_downscale) {
    // full-res placeholder buffer the BPS downscales from
    uint32_t full_stride, full_y_h, full_uv_h, full_yuv_size;
    std::tie(full_stride, full_y_h, full_uv_h, full_yuv_size) = get_nv12_info(sensor->frame_width, sensor->frame_height);
    bps_fullres_dummy.init(m, full_yuv_size, 0x1000, true, m->icp_device_iommu);

    // LUTs (matches upstream commaai/openpilot config_bps_downscale / #37876)
    assert(sensor->linearization_lut.size() == 36);
...
bool SpectraCamera::syncFirstFrame(int camera_id, uint64_t request_id, uint64_t raw_id, uint64_t timestamp) {
  ...
  bool all_cams_up = camera_sync_data.size() == running_cameras.size();
...
void SpectraCamera::classifyWideCamFault() {
  const int wide = WIDE_ROAD_CAMERA_CONFIG.camera_num;
  const int road = ROAD_CAMERA_CONFIG.camera_num;

  if (!WIDE_ROAD_CAMERA_CONFIG.enabled) return;
  if (camera_sync_data.count(wide) > 0) return;
  if (camera_sync_data.count(road) == 0) return;
  for (int cam : running_cameras) {
    if (cam != wide && camera_sync_data.count(cam) == 0) return;
  }

  wide_cam_fault = running_cameras.count(wide) > 0 ? WideCamFault::noFrames : WideCamFault::noBringup;
  LOGE("wide road camera faulty (%s), all other cameras are healthy",
       wide_cam_fault == WideCamFault::noFrames ? "no frames" : "no bringup");
}
