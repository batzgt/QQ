...
from iqpilot.system.ui.lib.screen_recorder import ScreenRecorder
...
_RAYLIB_BENIGN_WARNINGS = (
  ...
  "Failed to initialize color correction",
)
...
ASSETS_DIR = files("iqpilot.selfdrive").joinpath("assets")
...
class GuiApplication(IQAppHooks):
  ...
  def _apply_vsync_interval(self, fps: int) -> bool:
    if OFFSCREEN or not rl.is_window_ready() or not ENABLE_VSYNC:
      ...
      if EGL_DMA_BUF_SUPPORTED and rl.is_window_ready() and not OFFSCREEN:
        try:
          from iqpilot.system.ui.lib.egl import set_swap_interval
          set_swap_interval(0)
        except Exception:
          pass
      return False
  ...
  def _apply_display_sync_before_swap(self) -> None:
    ...
    try:
      ...
      from iqpilot.system.ui.lib.egl import finish_gl
  ...
  def _set_styles(self):
    ...
    rl.gui_set_style(rl.GuiControl.DEFAULT, rl.GuiDefaultProperty.BACKGROUND_COLOR, gui_style_color(rl.BLACK))
    rl.gui_set_style(rl.GuiControl.DEFAULT, rl.GuiControlProperty.TEXT_COLOR_NORMAL, gui_style_color(DEFAULT_TEXT_COLOR))
    rl.gui_set_style(rl.GuiControl.DEFAULT, rl.GuiControlProperty.BASE_COLOR_NORMAL, gui_style_color(rl.Color(50, 50, 50, 255)))
  ...
  def _set_log_callback(self):
    ...
    def trace_log_callback(log_level, text, args):
      ...
      text_str = text_str.replace("COMMA:", "IQ:")
