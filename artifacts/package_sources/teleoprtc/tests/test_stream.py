...
from teleoprtc.stream import _rtc_configuration_from_ice_servers
...
def test_explicit_empty_ice_servers_disable_defaults():
  assert _rtc_configuration_from_ice_servers(None) is None
  assert _rtc_configuration_from_ice_servers([]).iceServers == []
...
class TestOfferStream:
  async def test_offer_stream_sdp_recvonly_audio(self):
    ...
    builder = WebRTCOfferBuilder(capture, ice_servers=[])
    builder.offer_to_receive_audio_stream()
    stream = builder.stream()

    try:
      with pytest.raises(Exception, match="Offer captured"):
        await stream.start()
    finally:
      await stream.stop()
...
a=setup:actpass"""
    ...
    builder = WebRTCAnswerBuilder(offer_sdp, ice_servers=[])
