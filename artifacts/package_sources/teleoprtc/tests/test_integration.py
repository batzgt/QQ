...
class SimpleAnswerProvider:
  ...
  async def __call__(self, offer: StreamingOffer):
    ...
    builder = WebRTCAnswerBuilder(offer.sdp, ice_servers=[])
...
class TestStreamIntegration:
  @pytest.mark.asyncio
  @pytest.mark.parametrize("name,cameras,recv_audio,add_messaging", [
  ...
  async def test_multi_camera(self, name, cameras, recv_audio, add_messaging):
    ...
    offer_builder = WebRTCOfferBuilder(simple_answerer, ice_servers=[])
    for cam in cameras:
    ...
    try:
      ...
      await stream.stop()
      if simple_answerer.stream is not None:
