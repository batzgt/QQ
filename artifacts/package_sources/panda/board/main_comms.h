...
static int get_health_pkt(void *dat) {
  ...
  // Toyota SecOC hybrids keep the harness ignition line live in every gear, so reporting it
  // raw would mask the gear gated CAN ignition (Park would still read as ignition on). main.c
  // deliberately keeps bootkicking off the raw line, so the device still wakes up in Park.
  bool ignition_line = harness_check_ignition();
  if (ignition_can_gates_line && !ignition_can) {
    ignition_line = false;
  }
  health->ignition_line_pkt = ignition_line;
