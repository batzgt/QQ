...
static bool tesla_fsd_visualization = false;
...
static void tesla_rx_hook(const CANPacket_t *msg) {
  ...
  if (msg->bus == 2U) {
    ...
    if (msg->addr == 0x488U) {
      ...
      // Only consider rising edges while controls are not allowed
      if (!tesla_fsd_visualization && tesla_stock_steering_control_now && !tesla_stock_steering_control_prev && !is_lat_active()) {
        tesla_stock_steering_control = true;
      }
      if (tesla_fsd_visualization || !tesla_stock_steering_control_now) {
...
static bool tesla_fwd_hook(int bus_num, int addr) {
  ...
  if (bus_num == 2) {
    if (tesla_fsd_visualization && (addr == 0x488)) {
      block_msg = true;
    }
...
static safety_config tesla_init(uint16_t param) {
  ...
  const uint16_t TESLA_PARAM_IQ_FSD_VISUALIZATION = 2;

  tesla_has_vehicle_bus = GET_FLAG(current_safety_param_iq, TESLA_PARAM_IQ_VEHICLE_BUS);
  tesla_fsd_visualization = GET_FLAG(current_safety_param_iq, TESLA_PARAM_IQ_FSD_VISUALIZATION);
