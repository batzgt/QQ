...
from iqdbc.safety.tests.aol_common import AolSafetyTestBase
...
def parameterized_safety_class(attrs):
  def decorate(base_class):
    module = vars(sys.modules[base_class.__module__])
    existing_names = set(module)
    result = parameterized_class(attrs)(base_class)
    result.__test__ = False
    for name in set(module) - existing_names:
      generated_class = module[name]
      if isinstance(generated_class, type) and issubclass(generated_class, result):
        generated_class.__test__ = True
    return result
  return decorate
...
class SafetyTestBase(unittest.TestCase):
  ...
  def __init_subclass__(cls, **kwargs):
    ...
    cls.__test__ = cls.__name__.startswith("Test") and not cls.__name__.endswith("Base") and not inspect.isabstract(cls)
...
class TorqueSteeringSafetyTestBase(SafetyTestBase, abc.ABC):
  ...
  def test_steer_req_bit(self):
    ...
    if self.NO_STEER_REQ_BIT:
      ...
      self._set_prev_torque(0)
      self.assertTrue(self._tx(self._torque_cmd_msg(0, 1)))
      return
...
class SafetyTest(SafetyTestBase):
  ...
  def test_tx_hook_on_wrong_safety_mode(self):
    ...
    for tf in test_files:
      try:
        test = importlib.import_module("iqdbc.safety.tests."+tf[:-3])
      except ImportError:
        continue
...
class SecondSpeedSafetyTest(abc.ABC):
  @abc.abstractmethod
  def _speed_msg(self, speed):
    pass
